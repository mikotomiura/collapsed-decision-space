...
| [35] | The basis of the power discussion: conclusions in NLP often rest on designs without the power to support them. **An earlier version of this table called it load-bearing because the paper claimed the design was *not* underpowered; that claim is withdrawn** — the reported `power` is the nominal sensitivity of a surrogate, and the power of the decision's test is evaluated nowhere (§6.2) | §2 |
