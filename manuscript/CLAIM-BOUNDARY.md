...
- **At the registered `delta_tv` the surrogate returns `1.0000` for both bases checked**,
  ...
  (§4.3), finds the test's rejection rate equal to the surrogate's 1.0 from `delta_tv` = 0.05
...
- **Clearing a per-context entropy floor in every context does not certify the support that the
  ...
  **narrower and threshold-bound** — at the values fixed in §C.4, R4's two conditions are not met by
...
- **Neither of the two runs that produced an estimate returned one its own permutation test
  ...
  three independent measurements; §6.3 gives the reason agreement across runs of this apparatus
  would not establish what it appears to. This is a statement about what has been observed, not
  about the power of any test (§6.2), and not about whether the read-out could have detected a
  material shift.
...
| G20 | The Pearson surrogate is the power of the design or of its decision test; the design was adequately powered | `power` is the **nominal sensitivity of a surrogate**. The sealed design did not evaluate the power of the permutation test. The post hoc simulation of §4.3 reports its simulated rejection rate under declared, assumed bases; that is not the design being adequately powered, and both statements must stay sayable | `(design\|study\|protocol\|experiment) (was\|is) (adequately \|well \|sufficiently \|fully \|highly )?powered`, `power of the (stratified )?(label-)?permutation test (is\|was) (1\|0\.\d\|high\|full\|adequate\|sufficient\|above)`, `(design\|study) (had\|has\|achieved) (full \|adequate \|sufficient \|enough )?power`, `(design\|study\|protocol\|experiment) (had\|has) (enough\|adequate\|sufficient) sensitivity to detect` |
...
| G23 | The difference in `None` is caused by the channel, λ, locomotion, temperature or top_p | **Confounded with execution order**: every context ran its channel-on block first (§6.4) | `(channel\|λ\|lambda\|locomotion\|temperature\|top_p)[a-z ,]{0,20}(causes?\|caused\|drives?\|drove\|increases?\|increased\|raises?\|raised\|produces?\|produced) (more )?(none\|null\|unparse\|malformed\|format)`, `(none\|null\|unparse\w*\|malformed)[a-z ,]{0,40}(caused\|driven\|produced) by (the )?(channel\|λ\|lambda\|locomotion\|temperature\|top_p\|higher temperature)`, `(due\|attributable\|owing) to (the )?(channel\|λ\|lambda\|locomotion\|temperature\|top_p\|higher temperature)`, `channel[a-z -]{0,20}led to more (none\|null\|unparse\w*\|malformed\|format)` |
...
5. **A negative control holds the patterns away from sentences the manuscript must be able to
   ...
   estimand, the sealed note that the completed run "attained power 1.0", that the sealed design
   did not evaluate the power of the permutation test and what its post hoc simulation reports, the
   parser's design note that `None` means stay put, the
...
- Numbers the manuscript draws from the frozen inputs, and from the derived artefacts regenerated
  ...
  figures in §B.2 (timings, digests, peak memory, the projected run length) exist in no frozen input
...
- Quantities whose literals are too common for an occurrence test — small counts, zone counts, the
  rows of the tables in §4.1, §B.1 and the results section, the held-out counts and class
