...
| `src/erre_sandbox/integration/embodied/bank_scorer.py` (`scorer_schema_version = "ecl-cproper-scorer-1"`) | `analysis/apparatus/erre_sandbox/integration/embodied/bank_scorer.py` | **2026-09-12 訂正**: 「所在を特定できなかった」は誤り。原因は `paper/_closure.py` の `ENTRIES["02"]` に本モジュールの seed が無かったこと (前タスクの `grep -rn ... analysis/apparatus/` はコピー先しか見ておらず、原本 `src/erre_sandbox/integration/embodied/bank_scorer.py` の存在を見落としていた)。ENTRIES["02"] に seed を追加し (2026-09-12)、本ファイルを閉包へ同梱した。閉包は 68 → **69 ファイル**、`pwsh paper/gather.ps1 -Verify` は 02 分について **176 件中 176 MATCH / DRIFT 0 / MISSING 0** (一度 DRIFT 1 が出たが、原因は autocrlf による生データの正規化で本件とは別問題。原本から復元し `.gitattributes` で固定済 — 詳細は `data/data.md`)。`data/raw/cproper-verdict.json` が `scorer_schema_version="ecl-cproper-scorer-1"` を宣言しているのに scorer 本体が閉包に無いと、中核 verdict (`NO_CHANNEL_CONFORMANCE` を産んだ計算) を論文 repo 単体で再導出できなかった。それがこれで閉じた |
