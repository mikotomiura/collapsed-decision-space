...
証拠、測定 apparatus、そして完了済み run の verdict を同梱 draws から再計算し、protocol が引く
数値をその出所と突き合わせる 1 コマンドが入っています。**このコマンドが届かない範囲**は本文
§12.8 にあります — 前向きアームの verdict は入力として読まれるだけで、draw bundle が同梱されて
いないため再計算されません。
...
運んでいなかった理由が分かりました。検出力計算が台として使う channel-off 条件では 5 ゾーンの
うち 2 つが一度も産出されず、空セルが chi-square の検出力ゲートを膨らませ、距離統計量の
帰無床が宣言 margin の大半を食っています。
...
**その後、両アームを実走し、規則は R4 で止まりました。** 2 つ目の族では推定量が測定できない
ため、**この問いはこの実走では答えられていません**。実走が示したのは、per-context entropy floor
を全 context で通過しても、検出力計算が必要とする support は保証されないということです
(完了済み run も control アームも全 context で通過しています)。ゲートについて言えるのはより
狭く、**閾値に依存します** — 事前に固定した値のもとでは測定可能性ゲートはこの場合を flag
しませんが、同じ規則をより高い floor で使えば flag します (2 つ目の族では実際に発火しました)。
推定量を産出した 2 つの run は**どちらも `qwen3:8b`** で、いずれも自分自身の permutation test
が棄却していません。
