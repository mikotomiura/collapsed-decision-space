...
ため、**この問いはこの実走では答えられていません**。実走が示したのは、推定量の**前に**置かれた
妥当性ゲートがどこに届きどこに届かないかです — 完了済み run と control アームに当てはめると、
そのゲートの 2 条件はどちらも満たされません。本稿が主題にしているのは、まさにその regime です。
そして推定量を産出した run については、いずれも自分自身の permutation test が棄却していません。
...
- 推定量の前に置かれた妥当性ゲートは、推定量が測定できないアームには届き、**本稿が主題に
  している regime には届かない**こと — per-context entropy floor を全 context で通過することと、
  基底分布が 5 ゾーンのうち 2 つを欠くことは両立する
