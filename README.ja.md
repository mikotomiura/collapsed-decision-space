...
いなければ、そのステップ自身が「外側の半分は不在である」と出力します。通った場合でも
示せるのは、控えが**最後に触られた**時刻であって、封印ファイルが書かれた時刻ではありません
...
14. **封印ファイルを、アーカイブ自身が公開する checksum と突合**。3〜13 ステップ目は
    リポジトリ内部の記録どうしの照合であり、どちらの側も 1 人で書き換えられる。
    第三者が持つ控えと比べるのはこのステップだけである。ネットワークは使わない
    (記録済みの答えと手元の bytes を比べるだけ) ので de-identified な複製の中でも走る。
    `seal/zenodo-witness.json` が無い間は理由を明示して skip する。配線を先に入れる理由は
    13 と同じ。示せることは狭い：著者でない誰かがこれと同一の bytes を持ち、そう言った
    時刻を記録した、ということだけである。deposit は公開後しばらく識別子を変えずに
    所有者が差し替えられるので、その時刻が縛るのは「控えが最後に触られた時刻」であって、
    封印ファイルが書かれた時刻でも、それ以前に実走が無かったことでもない
...
- **封印が古いこと。** 12 ステップ目が示すのは、封印済みファイルが manifest の記録する bytes
  ...
  第三者が要ります。14 ステップ目がその突合であり、控えが記録されていなければそのステップ
  自身がそう出力します。通っても示せるのは控えが最後に触られた時刻までで、protocol は
  緑のバッジにそれを匂わせるのでなく、そう書いています
