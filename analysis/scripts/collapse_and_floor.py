...
**The permutation-null mean of the estimate.** Total variation is a non-negative distance, so its
expectation under the null is not zero. Permuting the condition labels within each context, which is
the same null the scorer's own permutation test uses, gives the distribution the observed value
should be read against. Its mean is a reference value for the observed statistic, not an additive
component of it: an earlier version of this script and of the manuscript called it a "floor" and
reported the share of the observed value it accounted for, and that reading is withdrawn. The sealed
``repro.sh`` still labels step 8 in the earlier vocabulary; its bytes are sealed, and so are the
script and output names it calls, which is why this file keeps its name.
...
def main() -> int:
    ...
    lines = [
        "# The support of the decision space, and the permutation-null mean of the estimate",
        "",
    ...
    lines += [
        ...
        "",
        "The power gate's pooled chi-square surrogate evaluated against this same empirical base, at the",
        f"sampling plan of the completed run (*M* = 300, *K* = 8, pooled), {POWER_REPLICATES} replicates, "
        f"seed {SEED}. This is the surrogate's sensitivity, not the power of the permutation test:",
        "",
